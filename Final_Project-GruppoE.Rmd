...
* *URBAN*: 1 se urbana, O se rurale (categoriale)
* *PRO*: 0 se non-profit, 1 altrimenti (categoriale)
* *TAXEXEMPT*: 1 se esente dalle tasse (categoriale)
* *SELFFUNDINS*: 1 se autofinanziato per l'assicurazione (categoriale)
* *MCERT*: 1 se certificato Medicare (categoriale)
...
```
Dai gafici dei residui si può afermare che l'omeschedasticità, la notmalità sono rispettati.
...
Nel primo gafico confrontiamo i dati predetti rispetto ai dati reali del test-set. Rispetto al modello precedente si osserva una maggiore dispersione.
...
La divisione del data-set fra quello di train e di test è casuale, quindi per controllare che non siamo stati fortunati nel caso che abbiamo studiato andremo a ripetere il processo 100 volte ed analizzare i risultati attraverso l'$R^2$ la varianza sui residui, l'errore relativo sui residui e i coefficienti ottenuti.
...
```
In giallo è segnato il coefficiente della regrassione consederando l'interezza dei dati.
...
plot_grid(p1, p2,
          p3, p4)
```
Dal precedente grafico possiamo capire che i coefficienti ottenuti nel nostro test si avvicinano molto al coefficienti reali (in giallo).
...
Visualizziamo le varianze within e between dei tre metodi:
